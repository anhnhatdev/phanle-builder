...
if (process.platform === 'win32') {
  app.setAppUserModelId(isDev ? `com.phanle.dev.${Date.now()}` : 'com.phanle.app');
}
...
//
// phanle://openChat?accountId=xxx&threadId=yyy&threadType=0&channel=zalo
...
function createWindow() {
  ...
  app.on('second-instance', (_event, argv) => {
    ...
    const deepLinkUrl = argv.find((arg: string) => arg.startsWith('phanle://'));
    if (deepLinkUrl) {
  ...
  ipcMain.on('app:setBadge', (_event, count: number) => {
    ...
    if (process.platform === 'darwin') {
      ...
      if (count > 0) {
        ...
        tray?.setToolTip(`phanle — ${count} tin chưa đọc`);
      } else {
        if (currentIconIsDot) {
...
/**
 * Xử lý deep link URL từ custom protocol phanle://
 *
 * Định dạng:
 *   phanle://openChat?accountId=xxx&threadId=yyy&threadType=0&channel=zalo
 *
...
app.whenReady().then(async () => {
  ...
  // ── Register phanle:// as default protocol client ─────────────────────
  // Cho phép OS mở app khi click link phanle:// trong trình duyệt
  //
  ...
  if (app.isPackaged) {
    if (!app.isDefaultProtocolClient('phanle')) {
      app.setAsDefaultProtocolClient('phanle');
    }
  }
