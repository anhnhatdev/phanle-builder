 ...
 *
 * Patches the Electron dev binary with phanle's icon and version metadata.
...
async function main() {
  ...
  try {
    ...
    await rcedit(electronExePath, {
      icon: iconPath,
      'version-string': {
        ProductName:     pkg.build?.productName || 'phanle',
        FileDescription: pkg.build?.productName || 'phanle',
        CompanyName:     'phanle',
      },
    ...
    console.log('[patch-icon] ✅ electron.exe patched with phanle icon');
  } catch (err) {
