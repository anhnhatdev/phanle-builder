...
export default function EmployeeLoginScreen({ onBossMode, onEmployeeConnected }: Props) {
    ...
    useEffect(() => {
        try {
            const saved = localStorage.getItem('phanle_employee_login');
            if (saved) {
    ...
    const handleEmployeeLogin = async () => {
        ...
        try {
            ...
            // Save for next time
            localStorage.setItem('phanle_employee_login', JSON.stringify({
            ...
            <div className="flex-1 flex items-center justify-center bg-gray-900 p-4">
                <div className="w-full max-w-md bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl overflow-hidden">
                    <div className="px-6 pt-6 pb-4 text-center">
                        <h1 className="text-xl font-bold text-white mb-1">phanle</h1>
                    ...
                    <div className="px-6 py-3 border-t border-gray-700/50 text-center">
                        <p className="text-[10px] text-gray-600">phanle — Quản lý Zalo & Facebook đa tài khoản</p>
                    </div>
                </div>
            </div>
        );
