...
const DEFAULT_DB_NAME = 'phanle-tool.db';           // existing DB
...
class WorkspaceManager {
     ...
     * Existing phanle-tool.db stays in place — the default workspace simply points to it.
     */
    private migrateFromLegacy(): void {
        ...
        const phanleConfigPath = path.join(this.userDataPath, 'phanle-config.json');
        if (fs.existsSync(phanleConfigPath)) {
            try {
                const cfg = JSON.parse(fs.readFileSync(phanleConfigPath, 'utf-8'));
    ...
    public createWorkspace(params: {
        ...
        //   workspace-{id}/phanle-tool.db + workspace-{id}/media/
        const wsFolder = `workspace-${id}`;
        const wsDbRelative = `${wsFolder}/phanle-tool.db`;
    ...
    public deleteWorkspace(id: string): { success: boolean; error?: string } {
        ...
        const wsDbPath = ws.dbPath || `workspace-${id}/phanle-tool.db`;
        ...
        // SAFETY: Never delete the root phanle-tool.db (belongs to default workspace)
     ...
     * Respects custom dbFolder from phanle-config.json.
     */
